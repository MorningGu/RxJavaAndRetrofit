...
public class XRecyclerView extends RecyclerView {
    ...
    private int scaleViewDefaultHeight = 383;
    private int refreshHeaderDefaultHeight = 160;
    ...
    private void init(Context context) {
        ...
        if(pullRefreshEnabled) {
            ...
            refreshHeaderDefaultHeight = refreshHeader.mMeasuredHeight;
    ...
    public void refreshComplete() {
        ...
        if(scaleView!=null){
            new Handler().postDelayed(new Runnable(){
                public void run() {
                    smoothScrollTo(scaleView.getMeasuredHeight(),scaleViewDefaultHeight,scaleView);
                }
            }, 500);
        }
    }
    ...
    public void setScaleView(View view,int height){
        scaleView = view;
        if(view!=null){
            scaleViewDefaultHeight = height;
        }
    }
    @Override
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ( isOnTop() && pullRefreshEnabled) {
                    ...
                    if(scaleView!=null){
                        ...
                        int height = layoutParams.height+ (int)(deltaY / DRAG_RATE);
                        if(height>scaleViewDefaultHeight){
            ...
            default:
                ...
                if ( isOnTop() && pullRefreshEnabled) {
                    if(scaleView!=null){
                        if(scaleView.getMeasuredHeight()>scaleViewDefaultHeight+refreshHeaderDefaultHeight){
                            smoothScrollTo(scaleView.getMeasuredHeight(),scaleViewDefaultHeight+refreshHeaderDefaultHeight,scaleView);
                        }else {
                            smoothScrollTo(scaleView.getMeasuredHeight(),scaleViewDefaultHeight,scaleView);
                        }
                    }
    ...
    private void smoothScrollTo(int currentHeight, int destHeight, final View view) {
        ValueAnimator animator = ValueAnimator.ofInt(currentHeight, destHeight);
        animator.setDuration(300).start();
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation)
            {
                setVisiableHeight(view,(int)animation.getAnimatedValue());
            }
        });
        animator.start();
    }
    private void setVisiableHeight(View view,int height){
